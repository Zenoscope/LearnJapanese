...
// draw the "search" box
var box_gap = 5; // gap around the text
...
draw_text_transformed(x - (spr_width - half_char_width), y - (box_height/2) , text, scale, scale,0);
...
if (blink) {
	...
	draw_line_width(
		...
		x - (spr_width - half_char_width) + (length * scale),
		y - ((height * 0.75) * scale) ,
		//x + (length * scale) + half_char_width,
		x - (spr_width - half_char_width) + (length * scale),
		y + ((height * 0.75) * scale), 
